...
def test_linear_congruential_success():
    ...
    # Dividido por m - 1 (99)
    generator = LCGenerator(semilla=37, a=19, c=33, m=100)
    ...
    # Probar con n = 0
    results_empty = generator.generate(0)
...
def test_linear_congruential_validation_errors():
    ...
    with pytest.raises(GeneratorValidationError, match="El módulo 'm' debe ser mayor que 0"):
        LCGenerator(semilla=37, a=19, c=33, m=0)

    # Semilla fuera de rango [0, m-1]
    with pytest.raises(GeneratorValidationError, match="La semilla .* debe estar en el rango"):
        LCGenerator(semilla=100, a=19, c=33, m=100)
    with pytest.raises(GeneratorValidationError, match="La semilla .* debe estar en el rango"):
        LCGenerator(semilla=-1, a=19, c=33, m=100)
    ...
    with pytest.raises(GeneratorValidationError, match="El multiplicador 'a' .* debe estar en el rango"):
        LCGenerator(semilla=37, a=100, c=33, m=100)
    with pytest.raises(GeneratorValidationError, match="El multiplicador 'a' .* debe estar en el rango"):
        LCGenerator(semilla=37, a=-1, c=33, m=100)
    ...
    with pytest.raises(GeneratorValidationError, match="El incremento 'c' .* debe estar en el rango"):
        LCGenerator(semilla=37, a=19, c=50, m=50)
    with pytest.raises(GeneratorValidationError, match="El incremento 'c' .* debe estar en el rango"):
        LCGenerator(semilla=37, a=19, c=-1, m=100)

    # Cantidad 'n' negativa
    with pytest.raises(GeneratorValidationError, match="La cantidad 'n' debe ser un entero no negativo"):
        LCGenerator(semilla=37, a=19, c=33, m=100).generate(-1)
...
def test_multiplicative_congruential_success():
    ...
    # X_0 = 17, a = 3, m = 100
    # Dividido por m - 1 (99)
    generator = MCGenerator(semilla=17, a=3, m=100)
...
def test_multiplicative_congruential_validation_errors():
    # Módulo <= 0
    with pytest.raises(GeneratorValidationError, match="El módulo 'm' debe ser mayor que 0"):
        MCGenerator(semilla=17, a=3, m=0)

    # Semilla fuera de rango [1, m-1] (la semilla no puede ser 0 en MCG)
    with pytest.raises(GeneratorValidationError, match="La semilla .* debe estar en el rango"):
        MCGenerator(semilla=0, a=3, m=100)
    with pytest.raises(GeneratorValidationError, match="La semilla .* debe estar en el rango"):
        MCGenerator(semilla=100, a=3, m=100)

    # Multiplicador 'a' fuera de rango [0, m-1]
    with pytest.raises(GeneratorValidationError, match="El multiplicador 'a' .* debe estar en el rango"):
        MCGenerator(semilla=17, a=100, m=100)
    with pytest.raises(GeneratorValidationError, match="El multiplicador 'a' .* debe estar en el rango"):
        MCGenerator(semilla=17, a=-1, m=100)

    # Cantidad 'n' negativa
    with pytest.raises(GeneratorValidationError, match="La cantidad 'n' debe ser un entero no negativo"):
        MCGenerator(semilla=17, a=3, m=100).generate(-1)
...
def test_mid_square_success():
    ...
    results = MidSquareGenerator(semilla=5735, digits=4).generate(5)
    assert results == [0.8902, 0.2456, 0.0319, 0.0176, 0.3097]

    # Probar con dígitos = 2
    # X_0 = 45, d = 2
    # X_0^2 = 2025 -> Dígitos medios: 02 => r_1 = 0.02
    # X_1 = 2 -> X_1^2 = 4 (rellenado a 4 caracteres: 0004) -> Dígitos medios: 04 => r_2 = 0.04
    results_two_digits = MidSquareGenerator(semilla=45, digits=2).generate(2)
