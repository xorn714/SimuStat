...
def test_uniform_continuous():
    u_list = [0.0, 0.5, 1.0]
    res = ContinuousDistributionGenerator.uniform(u_list, 10.0, 20.0)
    assert res == [10.0, 15.0, 20.0]

    with pytest.raises(GeneratorValidationError):
        ContinuousDistributionGenerator.uniform(u_list, 20.0, 10.0)


def test_uniform_continuous_rejects_out_of_range():
    # 1.5 no pertenece a [0, 1]
    with pytest.raises(GeneratorValidationError):
        ContinuousDistributionGenerator.uniform([0.0, 0.5, 1.5], 10.0, 20.0)
...
def test_normal_continuous():
    ...
    res = ContinuousDistributionGenerator.normal(u_list, 0.0, 1.0)
    assert len(res) == 4

    with pytest.raises(GeneratorValidationError):
        ContinuousDistributionGenerator.normal(u_list, 0.0, 0.0)


def test_weibull_continuous():
    u_list = [0.2, 0.8]
    res = ContinuousDistributionGenerator.weibull(u_list, 1.5, 2.0)
    assert len(res) == 2

    with pytest.raises(GeneratorValidationError):
        ContinuousDistributionGenerator.weibull(u_list, -1.0, 2.0)


def test_calculate_continuous_stats():
    vals = [10.0, 12.0, 14.0, 16.0, 18.0]
    stats, hist = ContinuousStatsCalculator.calculate_stats(
        vals, "uniform", {"a": 10.0, "b": 20.0}
    )
