...
interface ContinuousValuesTableProps {
  numbers: number[]
  continuousValues: number[]
}

export const ContinuousValuesTable: React.FC<ContinuousValuesTableProps> = ({
  numbers,
  continuousValues,
}) => {
  ...
  const combinedData = continuousValues.map((val, idx) => ({
    index: idx + 1,
    ...
    <div className="bg-[#171F33]/60 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white font-headline">Variables Aleatorias Continuas Generadas</h3>
            <p className="text-xs text-slate-400">
              Mostrando {filteredData.length} de {continuousValues.length} números generados (X_i)
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="bg-[#0F172A] border border-slate-800 text-slate-200 text-xs rounded-xl pl-9 pr-3 py-2 focus:outline-none focus:border-emerald-500 w-48"
            />
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-left text-xs text-slate-300">
          <thead className="bg-[#1E293B] text-slate-400 font-label uppercase sticky top-0 z-10">
            <tr>
              ...
              <th className="py-3 px-4 text-emerald-400">X_i (Variable Continua)</th>
            </tr>
          </thead>
            ...
            {paginatedData.length > 0 ? (
              paginatedData.map((row) => (
                <tr key={row.index} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="py-2.5 px-4 text-emerald-400 font-bold">{row.xVal}</td>
                </tr>
              ))
            ) : (
              <tr>
