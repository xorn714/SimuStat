...
def calculate_continuous_stats(
    ...
    # 1. Calcular media y varianza teóricas y función PDF según la distribución
    theo_mean = 0.0
    theo_var = 0.0
    ...
    if dist == "uniform":
        ...
        theo_mean = mean
        theo_var = std_dev ** 2
        pdf_func = lambda x: (1.0 / (std_dev * math.sqrt(2.0 * math.pi))) * math.exp(-0.5 * (((x - mean) / std_dev) ** 2))
        ...
        pdf_func = lambda x: (
            (beta / alpha) * ((x / alpha) ** (beta - 1.0)) * math.exp(-((x / alpha) ** beta))
            if x > 0 else 0.0
        )
    ...
    # 2. Generación de Histograma de Frecuencias y Densidad Teórica
    k = int(math.ceil(1.0 + 3.322 * math.log10(n)))
    k = max(5, min(k, 25))  # Entre 5 y 25 bins
